---
title: '8000020: Jak exportovat kontakty'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: 311964b1e8e049ad7c26b106d5fe551f2d348170
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694196"
---
# <a name="export-contacts"></a>Export kontaktů

1. V dolní části stránky v Outlooku na webu vyberte <img src='data:image/png;base64,' />
 **Uživatele**.
2. Na panelu nástrojů vyberte **Spravovat**  >  **Kontakty exportu**. 
3. Zvolte Exportovat všechny kontakty nebo jenom kontakty z určité složky a pak vyberte **exportovat**.
4. V dolní části stránky vyberte **Uložit** a uložte "contacts.csv" do výchozí složky stahování. Nebo vyberte šipku vedle tlačítka **Uložit** a vyberte **Uložit jako**.

Další informace najdete v článku [Export kontaktů z Outlooku](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

