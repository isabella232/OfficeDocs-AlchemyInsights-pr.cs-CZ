---
title: Vytvoření podpisu v Outlook na webu
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 550b1025213ae54c17aed9ae5829896d2c83a6196b3251bbd80a36e14e790b45
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 08/05/2021
ms.locfileid: "54040931"
---
# <a name="create-an-email-signature-in-outlook-on-the-web"></a>Vytvoření podpisu e-mailu v Outlook na webu

1. Přejít na <img src='data:image/png;base64,' />
 **Nastavení**  >  **Zobrazení všech Outlook nastavení**  >  **Psaní a odpovídání**.
1. V **části Podpis e-mailu** zadejte podpis a pomocí dostupných možností formátování změňte jeho vzhled.

*Poznámka:* Na každý účet můžete mít jenom jeden podpis.

Další informace najdete v tématu [Vytvoření a přidání podpisu e-mailu v Outlook na webu](https://support.office.com/article/5ff9dcfd-d3f1-447b-b2e9-39f91b074ea3).
