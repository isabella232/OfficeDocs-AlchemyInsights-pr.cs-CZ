---
title: '8000019: jak přidávat nebo importovat kontakty'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 7eb53224bb3370fc434e520d69910d0c9c23f501
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694052"
---
# <a name="import-contacts"></a>Import kontaktů

Před importem kontaktů do Outlooku na webu je budete muset exportovat od svého jiného poskytovatele e-mailu jako soubor. csv. Pak postupujte podle pokynů níže.

1. V dolní části stránky vyberte <img src='data:image/png;base64,' />
 **Uživatele**.
2. Na panelu nástrojů vyberte **Spravovat**  >  **Kontakty importu**.
3. Vyberte **Procházet**, vyberte soubor CSV a pak vyberte **otevřít**.
4. Vyberte **importovat**.

Další informace najdete v tématu [Import kontaktů do Outlooku](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

