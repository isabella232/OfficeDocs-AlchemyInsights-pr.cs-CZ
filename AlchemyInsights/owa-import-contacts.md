---
title: '8000019: Jak přidat nebo importovat kontakty'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999773"
---
# <a name="import-contacts"></a>Import kontaktů

Před importem kontaktů do Outlook na webu je budete muset exportovat od jiného poskytovatele e-mailu jako .csv souboru. Potom postupujte podle následujících pokynů.

1. V dolní části stránky vyberte <img src='data:image/png;base64,' />
 **Lidé**.
2. Na panelu nástrojů vyberte **Spravovat**  >  **import kontaktů**.
3. Vyberte **Procházet,** zvolte soubor CSV a pak vyberte **Otevřít.**
4. Vyberte **Importovat**.

Další informace najdete v tématu [Import kontaktů do Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

