---
title: Export kontaktů z Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "279"
- "8000020"
ms.assetid: 37fc1455-7324-4c76-9a94-085a45e94c51
ms.openlocfilehash: 2e58b91e174df5f8ec66db293f7c03160e802a75
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/14/2020
ms.locfileid: "47672193"
---
# <a name="export-your-contacts-in-outlookcom"></a>Export kontaktů do Outlook.com

1. Přejít na [lidi](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. V horní části stránky vyberte **Spravovat** \> **Export**.
3. Zvolte, jestli chcete exportovat všechny kontakty nebo jenom kontakty z určité složky, a vyberte **exportovat**.
4. V dolní části stránky vyberte **Uložit** a bude ve výchozí složce pro stahování uloženo "contacts.csv". Chcete-li zvolit, kam se má soubor. csv uložit, vyberte šipku vedle tlačítka Uložit a vyberte **Uložit jako**.

Další informace najdete v článku [Export kontaktů z Outlook.com do souboru CSV](https://support.office.com/article/578cca22-3550-4c73-b3f0-9978cfeac83f?wt.mc_id=Office_Outlook_com_Alchemy).